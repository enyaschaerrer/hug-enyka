            ...
            <div v-else-if="kpis" class="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                >
                    ...
                    <template v-else-if="card.abandonSteps !== undefined">
                        ...
                        <div v-if="card.available && card.companies && card.companies.length > 0" class="relative mt-3 -mb-5">
                            <div class="min-h-48 max-h-48 no-scrollbar overflow-y-auto pr-1 pb-2 pt-2 border-t border-[var(--color-martinique-100)]" @scroll="onListScroll($event, card.label)">
                                <div
                            ...
                            <div
                                class="pointer-events-none absolute bottom-0 left-0 right-0 h-14 transition-opacity duration-300"
                                :style="{ background: `linear-gradient(to top, var(--color-martinique-50), transparent)` }"
                    ...
                    <template v-else-if="card.companies !== undefined">
                        ...
                        <div v-if="card.available && card.companies.length > 0" class="relative mt-3 -mb-5">
                            <div class="min-h-48 max-h-48 no-scrollbar overflow-y-auto pr-1 pb-2 pt-2 border-t" :class="card.isCobranded ? 'border-[var(--color-martinique-100)]' : 'border-base-200'" @scroll="onListScroll($event, card.label)">
                            <div
                                v-for="company in filterCompanies(card.companies)"
                                :key="company.name"
                                class="flex cursor-pointer items-center gap-3 py-2"
